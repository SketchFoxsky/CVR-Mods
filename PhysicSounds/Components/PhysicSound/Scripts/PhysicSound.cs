...
namespace Sketch.PhysicSounds
{
    ...
    public class PhysicSound : MonoBehaviour
    {
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            if(UseRandomPitch)
                AudioSourceReference.pitch = (Random.Range(Pitch.x, Pitch.y));

            if ((ColMag >= 0.1f) && (ColMag <= 4.9))
            {
                var clip = MinCollisionAudio[Random.Range(0, MinCollisionAudio.Length)];
                //Debug.Log("Small");
                AudioSourceReference.PlayOneShot(clip);
            }
            if ((ColMag >= 5f) && (ColMag <= 10))
            {
                var clip = MiddleCollisionAudio[Random.Range(0, MinCollisionAudio.Length)];
                //Debug.Log("Mid");
                AudioSourceReference.PlayOneShot(clip);
            }
            if (ColMag >= 10.01f)
            {
                var clip = MaxCollisionAudio[Random.Range(0, MinCollisionAudio.Length)];
                //Debug.Log("Large");
                AudioSourceReference.PlayOneShot(clip);
            }
        }
