...
namespace Sketch.ClimbingSystem
{
    public class ClimbManager : MonoBehaviour
    {
        ...
        private Vector3 deskLocalOffset;      // vector in anchor.xf local space from anchor point to player center

        // Desktop tween for anchor latch/switch
        private bool deskLerpActive;
        private float deskLerpT;
        private Vector3 deskLerpStart;        // world start pos of the lerp

        // ===== VR (single active hand, hysteresis, anti-ping-pong, moving anchors) =====
        ...
        // ---------------- Anchor type (supports moving & rotating climbables) ----------------
        ...
        private void HandleDesktop()
        {
            ...
            // If we're mid-latch tween, glide to new anchor; ignore WASD until done
            if (deskLerpActive)
            {
            ...
            // Normal latched movement: camera-relative WASD modifies the LOCAL offset (so it rotates with the anchor)
            Vector3 inputXZ = CVRInputManager.Instance.movementVector; // (x,z)
